...
class AStar:
    ...
    def get_route(
        ...
        if callable(getattr(self._routing_strategy, "route", None)):
            ...
            return shortest_path_coords, self._routing_strategy.route_length(
                shortest_path_ids, elevation=False
            )
        ...
        while queue:
            ...
            for neighbor in adj[cur]:
                neighbor_cost = costs[cur] + self._routing_strategy.weight(
                    cur, neighbor, None
                )
                if neighbor not in costs or neighbor_cost < costs[neighbor]:
                    came_from[neighbor] = cur
                    costs[neighbor] = neighbor_cost
                    fscore = neighbor_cost + self._routing_strategy.heuristic(
                        neighbor, end
                    )
...
class DirectStrategy(RoutingStrategy):
    """Direct distance routing strategy"""

    def __init__(self, graph, max_dist):
        super().__init__(graph, max_dist)

    def heuristic(self, node, target):
        """Heuristic is the haversine distance between two nodes"""
    ...
    def weight(self, node1, node2, edge_attrs):
        """Weight is the haversine distance between two nodes"""
...
class MinimizeStrategy(RoutingStrategy):
    """Strategy to minimize elevation gain"""

    def __init__(self, graph, max_dist):
        super().__init__(graph, max_dist)

    def heuristic(self, node, target):
        """Heuristic is the harversine distance between the two nodes"""
        n1 = (self.graph.nodes[node]["y"], self.graph.nodes[node]["x"])
        n2 = (self.graph.nodes[target]["y"], self.graph.nodes[target]["x"])
        hav = haversine(n1, n2)
    ...
    def weight(self, node1, node2, edge_attrs):
        """Weight is the elevation gain between the two nodes"""
...
class MaximizeStrategy(RoutingStrategy):
    """Strategy to maximize elevation gain"""

    def __init__(self, graph, max_dist):
        super().__init__(graph, max_dist)

    def heuristic(self, node, target):
        """Heuristic is the haversine distance between the two nodes"""
        n1 = (self.graph.nodes[node]["y"], self.graph.nodes[node]["x"])
        n2 = (self.graph.nodes[target]["y"], self.graph.nodes[target]["x"])
        hav = haversine(n1, n2)
    ...
    def weight(self, node1, node2, edge_attrs):
        """Weight is the haversine distance between the two nodes"""
        n1 = (self.graph.nodes[node1]["y"], self.graph.nodes[node1]["x"])
        n2 = (self.graph.nodes[node2]["y"], self.graph.nodes[node2]["x"])
        hav = haversine(n1, n2)
    ...
    def route(self, start, end) -> list[int]:
        """
        Find a route from start to end that maximizes elevation gain.
        ...
        for i in range(1, len(direct)):
            direct_lens[i] = direct_lens[i - 1] - (
                self.heuristic(direct[i], direct[i - 1])
            )
        max_length = (
            direct_len * self.max_dist
        ) - direct_len  # maximum amount of length we can add on
        new_path = []
        for i in range(len(direct) - 1):
            ...
            for path in nx.all_simple_paths(self.graph, prev, next_, 8):
                ...
                lim = (
                    max_length - direct_lens[i]
                )  # limit on how much the current node can increase distance-wise
...
def getStrategy(mode: str, max_dist: int) -> RoutingStrategy:
    ...
    :param max_dist: The maximum distance to travel
